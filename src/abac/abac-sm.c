...
void abac_state_initial(ogs_fsm_t *s, abac_event_t *e)
{
    ...
    OGS_FSM_TRAN(s, &abac_state_operational);
}
...
void abac_state_operational(ogs_fsm_t *s, abac_event_t *e)
{
    ...
    switch (e->h.id) {
        ...
        case OGS_EVENT_SBI_SERVER:
            ...
            abac_nabac_dr_handle_subscription_authentication(
                    stream, &message);
            break;

            CASE(OGS_SBI_RESOURCE_NAME_CONTEXT_DATA)
            abac_nabac_dr_handle_subscription_context(stream, &message);
            break;
            ...
            abac_nabac_dr_handle_subscription_provisioned(
                    stream, &message);
            break;
            DEFAULT
            ogs_error("Invalid HTTP method [%s]",
            ...
            abac_nabac_dr_handle_policy_data(stream, &message);
            break;
        ...
        default:
            ogs_error("No handler for event %s", abac_event_get_name(e));
            break;
    }
}